...
; Author: Thibault Wafflard & Anton Lamotte
...
(defun fux-cp-6th (&optional (species 6))
    "Create the CSP for the first species of Fux's three voices counterpoint."
    ...
    (add-no-unisson-cst (first *cp2) *cf)
    (print *cf)
    (print (first *cp))
    (print (first *cp2))
    (print *total-cp)

    ;(gil::distinct *sp* (list (first *cp) (first *cp2)))
