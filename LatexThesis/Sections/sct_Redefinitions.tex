...
\paragraph{}
When a variable is not explicitly linked to a voice, it is implied that the relation expressed for it is true for all \textit{parts}. In other words, if the variable $X$ is written without any precision, it means $X \equiv \forall v \in \{cf, cp_1, cp_2\}: X(v)$.
...
\subsection{Redefined variables} \label{subsection:modified_variables}
