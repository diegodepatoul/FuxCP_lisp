...
It will consist of seven subsections: first, a section for the implicit rules (the rules that Fux doesn't mention, but are present in all his examples), then a section for each species, and finally one that considers the interaction between different species. There is no section on rules for all species, yet there \textit{are} rules that apply to all species. In fact, all the rules mentioned in the first species section also apply to the other species. This isn't something that Fux mentions explicitly, but it's obvious when you look at how he teaches these rules and how he composes them. Therefore, the rules for all species are in the first species section. The reason these rules are in the first type section is because Fux himself mentions these rules in his chapter on first type counterpoints.
...
\paragraph{}
Please note that all the rules from T. Wafflard's thesis still apply to three-part compositions. The numbering of the rules in this work is the same as in T. Wafflard's work. If a rule is defined with the same number as an existing rule for two-part compositions, this means that the corresponding rule from T. Wafflard's work does not apply to three-part compositions, and that the new rule should be used instead. To make this clearer, there is a green dot (\greendot) next to these rules.
...
\subsection{Formalisation into constraints} \label{sec:generalconstraints}
    \paragraph{\ref{rule:last-chord-not-perfect-anymore}} \greendots \textit{First and last notes have not to be perfect consonances anymore.}
...
Concerning the rules applying to all species (those marked with a red dot): when a constraint described in this section applies to all species, it means that it applies to the first beat of every species, unless mentioned otherwise, and excepted for the fourth species, where it applies to the third beat.
...
\begin{enumerate}[wide, label=\bfseries 1.H\arabic*]
   ...
   This rule is an update of previous \textbf{1.H1} (that previously was saying that \textit{all} intervals must be consonants). Fux states that the upper voices and the lowest one are consonant, and not all voices together. 
    ...
    %start template
    \item\label{rule:harmonic-triad}  \textit{The harmonic triad should be used as much as possible.}
    ...
    \item\label{rule:unison-vs-octave}  \textit{Octaves should be used over unisons.}
...
\setcounter{enumi}{2} % Start from 3
    ...
    \item\label{rule:variety}  \textit{The notes of each part should be as diverse as possible.}
...
\begin{enumerate}[wide, label=\bfseries 1.P\arabic*]
    \item\label{rule:direct-to-p-cons} \greendots  \textit{Reaching a perfect consonance by direct motion should be avoided.}
...
\setcounter{enumi}{3} % Start from 4
    \item\label{rule:succ-p-cons}   \textit{Successive perfect consonances are prohibited.}
...
\paragraph{\hspace{.6cm}\ref{rule:consonant}} \greendots \textit{All notes on the downbeat are consonant with the notes of the lowest stratum.}
    ...
    \paragraph{\hspace{.6cm}\ref{rule:harmonic-triad}}  \textit{The harmonic triad should be used as much as possible.}
...
\paragraph{\hspace{.6cm}\ref{rule:variety}}  \textit{The notes of each part should be as diverse as possible.}
...
\subsubsection{Motion constraints}
\paragraph{\hspace{.6cm}\ref{rule:direct-to-p-cons}} \greendots \textit{Reaching a perfect consonance by direct motion should be avoided.}
...
\paragraph{\hspace{.6cm}\ref{rule:succ-p-cons}}   \textit{Successive perfect consonances are prohibited.}
    ...
    This is not a strict rule but an indication to make easier for the composer to have contrary motions. Given that is not an obligation, nor a preference, it was simply added as a heuristic for the solver. This is discussed in section \ref{heuristics}.
...
\setcounter{enumi}{1} % Start from 4
    \item \greendots \textit{It is allowed to ligature the fourth-to-last with the third-to-last or to ligature the third-to-last with the second-to-last.} \label{rule:2nd-species-ligatures}    
    \begin{quotation}
...
\subsubsection{Melodic constraints}
    ...
    \paragraph{\hspace{.6cm}\ref{rule:2nd-species-ligatures}} \greendots \textit{It is allowed to ligature the fourth-to-last with the third-to-last or to ligature the third-to-last with the second-to-last.}  
...
As has been discussed in the English section, there is no constraint to add for this rule, which isn't really a rule.
